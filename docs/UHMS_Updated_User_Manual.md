...
NHIS is treated as one of many insurance types. It is not given special handling outside of normal insurance configuration.
...
Investigation staff must select the requested items they intend to perform before accepting.

- Only accepted selected items are billed to the visit invoice.
- Unselected items are not billed.
- Once results have been entered for an item, the doctor cannot delete that investigation item.
...
Balance and payment status (both pricing modes):
...
- `discount_amount` is not automatically calculated from insurance.
