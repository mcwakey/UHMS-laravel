...
- **Broad relevant suite** (`Workspace|Admission|Ward|Nursing|Emergency|Records|Inpatient|Discharge|Bed|Visit|Consultation|RoleDashboard`): **675 passed / 43 failed**. The failures are pre-existing (they also fail with this pass's changes stashed — see §16) and are concentrated outside the inpatient workspace surface; every inpatient-workspace, ward-view and admission-workflow class passes.

## 16. Remaining limitations

- 43 pre-existing failures in the broad net (consultation-stabilisation and related suites) predate this work and are tracked separately; none touch the inpatient workspace surface (all inpatient/ward/bed/admission classes are green).
