...
| 14R.8 | Snapshot completion-occurrence identity hardening ✅ | low | ✅ **P2 closed**; 28 new tests (41 checks); hashes byte-identical; no flag enabled; no billing change |

**14R.8 is complete. Risk P2 is `CLOSED_BY_PHASE_14R_8`: consultation completion now allocates a durable completion-occurrence ULID inside its transaction, and snapshot identity binds to that occurrence instead of to `completed_at`. Three completions inside one frozen second produce v1, v2 and v3. Existing snapshot payloads, hashes and versions are unchanged, historical rows are not backfilled, and no flag, billing behaviour or clinical field changed. Browser E2E (R1), the wide-suite memory limit (R2) and clinician acceptance remain open exactly as before. See `OBGYN_MATERNITY_SNAPSHOT_COMPLETION_IDENTITY_PHASE_14R_8_REPORT.md`.**

**14R.7 is complete. Gate 0 (preview-modal parity) is closed, isolated pilot data and manifest-driven cleanup exist, the environment reconciliation dry run is captured, and a read-only preflight reports pilot readiness. The honest verdict is `READY_FOR_CLINICAL_PILOT` — no clinician has tested this yet, and risk P2 (same-second recompletion) is confirmed *(since closed by 14R.8)*. See `OBGYN_MATERNITY_PILOT_REGRESSION_PHASE_14R_7_REPORT.md`.**
