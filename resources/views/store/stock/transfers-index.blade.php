@extends('layouts.app')
@section('title', __('stock.stock_transfers'))

@section('content')
<div class="d-flex align-items-sm-center flex-sm-row flex-column gap-2 pb-3 mb-3 border-bottom">
    <div class="flex-grow-1">
        <h4 class="fw-bold mb-0">{{ __('stock.stock_transfers') }}
            <span class="badge badge-soft-primary border border-primary fs-13 fw-medium ms-2">{{ __('stock.total') }}: {{ $batches->total() }}</span>
        </h4>
        <small class="text-muted">{{ __('stock.transfers_description') }}</small>
    </div>
    <div>
        @can('store.purchase.create')
        <a href="{{ route('admin.store.stock.transfers.create') }}" class="btn btn-primary btn-md fs-13"><i class="ti ti-plus me-1"></i>{{ __('stock.new_transfer') }}</a>
        @endcan
    </div>
</div>
...
<div class="card">
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            @empty
                <tr><td colspan="7"><x-empty-state icon="ti-transfer" title="{{ __('stock.no_transfers') }}" message="{{ __('stock.no_transfers_message') }}" /></td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
