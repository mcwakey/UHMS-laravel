@extends('layouts.app')
@section('title', __('stock.new_stock_return'))

@section('content')
<div class="d-flex align-items-sm-center flex-sm-row flex-column gap-2 pb-3 mb-3 border-bottom">
    <div class="flex-grow-1">
        <h4 class="fw-bold mb-0">{{ __('stock.new_stock_return') }}</h4>
        <small class="text-muted">{{ __('stock.return_description') }}</small>
    </div>
    <div>
        <a href="{{ route('admin.store.stock.returns.index') }}" class="btn btn-outline-secondary"><i class="ti ti-arrow-left me-1"></i>{{ __('stock.back') }}</a>
    </div>
</div>
...
@if(!$mainStore)
<div class="alert alert-warning"><i class="ti ti-alert-triangle me-1"></i>{{ __('stock.no_main_store_return') }}</div>
@elseif($managedLocations->isEmpty())
<div class="alert alert-warning"><i class="ti ti-alert-triangle me-1"></i>{{ __('stock.no_managed_locations_return') }}</div>
@else
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.store.stock.returns.store') }}" id="returnForm">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    <label class="form-label">{{ __('stock.return_from') }} <span class="text-danger">*</span></label>
                ...
                <div class="col-md-2">
                    <label class="form-label">{{ __('stock.return_to') }}</label>
            ...
            <h6 class="mb-2">{{ __('stock.return_lines') }} <span class="text-danger">*</span></h6>
            ...
            <div>
                <button type="submit" class="btn btn-primary"><i class="ti ti-arrow-back-up me-1"></i>{{ __('stock.record_returns') }}</button>
                <a href="{{ route('admin.store.stock.returns.index') }}" class="btn btn-light">{{ __('stock.cancel') }}</a>
            </div>
        </form>
    </div>
</div>
