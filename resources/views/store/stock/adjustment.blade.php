...
<div class="d-flex align-items-sm-center flex-sm-row flex-column gap-2 pb-3 mb-3 border-bottom">
    <div class="flex-grow-1">
        ...
        <small class="text-muted">For corrections, damaged stock, expired stock, or physical count differences.</small>
    </div>
...
<form method="POST" action="{{ route('admin.store.stock.adjustments.store') }}">@csrf
    <div class="row g-3">
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <select name="type" class="form-select" required>
                <option value="in"       {{ old('type')==='in'      ?'selected':'' }}>Adjustment IN (add stock)</option>
                <option value="out"      {{ old('type')==='out'     ?'selected':'' }}>Adjustment OUT (remove stock)</option>
                <option value="damaged"  {{ old('type')==='damaged' ?'selected':'' }}>Damaged (remove)</option>
                <option value="expired"  {{ old('type')==='expired' ?'selected':'' }}>Expired (remove)</option>
            </select>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="reason" class="form-control" value="{{ old('reason') }}" placeholder="e.g. Physical count variance, water damage..." required>
        </div>
        <div class="col-md-6">
    ...
    <hr class="my-4">
    <button type="submit" class="btn btn-warning"><i class="ti ti-check me-1"></i>Record Adjustment</button>
