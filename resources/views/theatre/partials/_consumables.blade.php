...
<hr>
<h6 class="text-uppercase text-muted small mb-2">{{ __('theatre.consumables_used') }}</h6>
<div class="table-responsive">
    <table class="table table-sm align-middle mb-1" id="consumables-{{ $stageId }}">
        <thead class="table-light">
            <tr><th style="width:55%">{{ __('theatre.product') }}</th><th style="width:15%">{{ __('theatre.qty') }}</th><th>{{ __('theatre.notes') }}</th><th style="width:32px"></th></tr>
        </thead>
        <tbody>
            @forelse(($defaultConsumables ?? collect()) as $i => $sc)
                <tr>
                    <td>
                        ...
                        @if($sc->is_required)<span class="badge bg-warning-subtle text-warning ms-1">{{ __('theatre.required') }}</span>@endif
                    </td>
                    ...
                    <td class="text-end">
                        <button aria-label="{{ __('common.close') }}" title="{{ __('common.close') }}" type="button" class="btn btn-sm btn-outline-danger" onclick="this.closest('tr').remove();"><i class="ti ti-x"></i></button>
                    </td>
                </tr>
            @empty
            @endforelse
    ...
    <small class="text-muted d-block">{{ __('theatre.consumables_hint') }}</small>
</div>
