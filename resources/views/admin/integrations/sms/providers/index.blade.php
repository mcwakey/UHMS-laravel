...
@section('title', __('sms.sms_providers'))

@section('content')
<div class="page-wrapper">
    <div class="content">

        <x-page-header :title="__('sms.sms_gateway')" :description="__('sms.sms_providers')" icon="ti-message-2">
            <x-slot:actions>
                @can('integrations.sms.send')
                    <a href="{{ route('admin.integrations.sms.messages.create') }}" class="btn btn-outline-primary">
                        <i class="ti ti-send me-1"></i>{{ __('sms.send_test_sms') }}
                    </a>
                @endcan
                @can('integrations.sms.providers.manage')
                    <a href="{{ route('admin.integrations.sms.providers.create') }}" class="btn btn-primary">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-0">
                @if($providers->isEmpty())
                    <x-empty-state icon="ti-message-2" :title="__('integrations.no_active_provider')" :message="__('sms.no_messages')" />
                @else
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <div class="d-inline-flex gap-1">
                                                @can('integrations.sms.test')
                                                    <x-confirm-form :action="route('admin.integrations.sms.providers.test', $provider)"
                                                        buttonClass="btn btn-sm btn-outline-secondary" icon="ti-plug-connected"
                                                        :buttonLabel="__('integrations.test')"
                                                        :confirmTitle="__('integrations.test_connection')"
                                                        :confirmText="__('integrations.test_connection')" />
                                                @endcan
                                                @can('integrations.sms.providers.activate')
                                                    @if($provider->is_active)
                                                        <x-confirm-form :action="route('admin.integrations.sms.providers.deactivate', $provider)"
                                                            buttonClass="btn btn-sm btn-outline-warning" icon="ti-plug-off"
                                                            :buttonLabel="__('integrations.deactivate')"
                                                            :confirmTitle="__('integrations.deactivate_provider')"
                                                            :confirmText="__('integrations.deactivate_provider')" />
                                                    @else
                                                        <x-confirm-form :action="route('admin.integrations.sms.providers.activate', $provider)"
                                                            buttonClass="btn btn-sm btn-outline-success" icon="ti-plug-connected"
                                                            :buttonLabel="__('integrations.activate')"
                                                            :confirmTitle="__('integrations.activate_provider')"
                                                            :confirmText="__('integrations.activation_warning')" />
                                                    @endif
                                                @endcan
                                                @can('integrations.sms.providers.manage')
                                                    <a href="{{ route('admin.integrations.sms.providers.edit', $provider) }}" class="btn btn-sm btn-outline-primary">
