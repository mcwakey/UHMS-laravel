...
@php($canonicalDisplayLabel = $editing ? (($canonicalSectionLabels ?? [])[$section->section_key] ?? null) : null)
@if($editing)
    <td>
        ...
        @if($aliasTarget)
            <div class="mt-1 d-flex flex-wrap gap-1" title="{{ __('consultation_specialties.admin.hidden_from_doctor_workspace') }}">
                ...
                <span class="badge bg-info-subtle text-info">{{ __('consultation_specialties.admin.maps_to', ['section' => ($canonicalSectionLabels[$aliasTarget] ?? __('consultation_specialties.sections.'.$aliasTarget))]) }}</span>
            </div>
        @endif
