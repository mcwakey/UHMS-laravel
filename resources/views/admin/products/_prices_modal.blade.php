            ...
            <form method="POST" action="{{ route('admin.products.pricing.store', $product) }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-4 pb-3 border-bottom">
                        <div class="col-md-6">
                            <label class="form-label fw-medium small">{{ __('products.base_price_cash_short') }}</label>
                            ...
                            <small class="text-muted">{{ __('products.base_price_fallback_help') }}</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-medium small d-block">{{ __('products.billable_question') }}</label>
                            <div class="form-check form-switch">
                                ...
                                <label class="form-check-label" for="isBillable-{{ $product->id }}">
                                    {{ __('products.include_in_invoices') }}
                                </label>
                            </div>
                            <small class="text-muted">{{ __('products.non_chargeable_supplies_help') }}</small>
                        </div>
                        <div class="col-12">
                            <button type="submit" form="basePriceForm-{{ $product->id }}"
                                    class="btn btn-sm btn-outline-primary">
                                <i class="ti ti-device-floppy me-1"></i>{{ __('products.update_base_price') }}
                            </button>
                        </div>
                    </div>

                    {{-- Default prices per type --}}
                    <h6 class="fw-bold mb-1">{{ __('products.default_prices_by_insurance_type') }}</h6>
                    <p class="text-muted small mb-3">
                        {{ __('products.default_prices_modal_help', ['price' => '₵'.number_format($basePrice, 2)]) }}
                    </p>
                    ...
                    <h6 class="fw-bold mb-1">{{ __('products.provider_overrides') }}</h6>
                    <p class="text-muted small mb-3">
                            ...
                            <div class="row g-2 align-items-end mb-2 provider-price-row">
                                ...
                                <div class="col-md-1 d-flex align-items-end pb-1">
                                    <a href="#"
                                       ...
                                       class="btn btn-sm btn-outline-danger" aria-label="{{ __('common.delete') }}" title="{{ __('common.delete') }}">
