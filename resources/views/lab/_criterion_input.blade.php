...
<div class="row g-2 mb-2 align-items-end">
    <div class="col-md-5">
        ...
        @switch($c->input_type)
            @case('number')
                <input type="number" step="any" name="values[{{ $c->id }}][value]" class="form-control form-control-sm" value="{{ $criterionValue }}" @if($c->is_required) required @endif>
                @break
            @case('select')
                ...
                @break
            @case('textarea')
                <textarea name="values[{{ $c->id }}][value]" class="form-control form-control-sm" rows="2" @if($c->is_required) required @endif>{{ $criterionValue }}</textarea>
                @break
            @case('boolean')
                ...
                @break
            @default
                <input type="text" name="values[{{ $c->id }}][value]" class="form-control form-control-sm" value="{{ $criterionValue }}" @if($c->is_required) required @endif>
        @endswitch
