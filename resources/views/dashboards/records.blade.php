@extends('layouts.app')
@section('title', ($workspaceContext['workspaceKey'] ?? null) === 'records' ? __('records.dashboard.title') : __('role_dashboards.receptionist.title'))
...
<div class="d-flex align-items-sm-center justify-content-between flex-wrap gap-2 mb-4">
    <h4 class="fw-bold mb-0">{{ ($workspaceContext['workspaceKey'] ?? null) === 'records' ? __('records.dashboard.title') : $t('title') }}</h4>
    <div class="d-flex gap-2">
        @can('appointments.create')
            <a href="{{ $workspaceRoutes->route('admin.appointments.create') }}" class="btn btn-primary"><i class="ti ti-plus me-1"></i>{{ $t('new_appointment') }}</a>
        @endcan
        @can('patients.create')
            <a href="{{ $workspaceRoutes->route('admin.patients.create') }}" class="btn btn-outline-dark"><i class="ti ti-user-plus me-1"></i>{{ $t('register_patient') }}</a>
        @endcan
    </div>
</div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        @forelse($todaysAppointments as $appointment)
                            <tr>
                                ...
                                <td class="text-end pe-3"><a href="{{ $workspaceRoutes->route('admin.appointments.index') }}" class="btn btn-sm btn-outline-secondary">{{ $tc('view') }}</a></td>
                            </tr>
                        @empty
    ...
    <div class="col-xl-4">
        <div class="card border shadow-sm h-100 mb-0">
            ...
            <div class="card-body">
                ...
                @endforelse
                <a href="{{ $workspaceRoutes->route('admin.visits.index') }}" class="btn btn-light w-100 mt-2">{{ $tc('view_full_queue') }}</a>
            </div>
        </div>
    </div>
    @endcan
    ...
    <div class="col-xl-8">
        <div class="card border shadow-sm h-100 mb-0">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <a href="{{ $workspaceRoutes->route('admin.appointments.index') }}" class="btn btn-sm btn-outline-secondary">{{ $tc('view_all') }}</a>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        @forelse($appointmentRequests as $request)
                            <tr>
                                ...
                                <td class="text-end pe-3"><a href="{{ $workspaceRoutes->route('admin.appointments.index') }}" class="btn btn-sm btn-primary">{{ $tc('view') }}</a></td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="text-center text-muted py-4">{{ $tc('no_data') }}</td></tr>
                        @endforelse
