@extends('layouts.app')
@section('title', $meta['title'] ?? __('reports.hub_title'))
...
<form method="GET" class="card mb-3">
    <div class="card-body">
        <div class="row g-2 align-items-end">
            <div class="col-md-2"><label class="form-label">{{ __('reports.date_from') }}</label><input type="date" name="date_from" value="{{ $filters['date_from'] ?? '' }}" class="form-control"></div>
            <div class="col-md-2"><label class="form-label">{{ __('reports.date_to') }}</label><input type="date" name="date_to" value="{{ $filters['date_to'] ?? '' }}" class="form-control"></div>
            <div class="col-md-2"><label class="form-label">{{ __('reports.status') }}</label><input name="status" value="{{ $filters['status'] ?? '' }}" class="form-control" placeholder="{{ __('reports.filter') }}"></div>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                <label class="form-label">{{ __('reports.user_staff') }}</label>
            ...
            <div class="col-md-2 d-flex gap-2">
                <button class="btn btn-primary w-100">{{ __('reports.run') }}</button>
                <a class="btn btn-outline-secondary" href="{{ route('admin.reports.'.$key) }}">{{ __('reports.clear') }}</a>
            </div>
        </div>
        @if($key === 'blood-bank')
            <div class="row g-2 mt-1">
                <div class="col-md-2">
                    <label class="form-label">{{ __('reports.blood_group') }}</label>
                    <select name="blood_group" class="form-select">
                        <option value="">{{ __('reports.all_groups') }}</option>
...
<div class="row g-3 mb-3">
    <div class="col-6 col-xl-2"><div class="card border-0 bg-light"><div class="card-body py-3"><div class="small text-muted">{{ __('reports.total_rows') }}</div><div class="h4 mb-0">{{ $summary['total'] ?? 0 }}</div></div></div></div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="{{ count($columns) }}" class="text-center text-muted py-4">{{ __('reports.no_data') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
