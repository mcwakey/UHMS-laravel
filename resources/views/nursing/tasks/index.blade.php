...
<div class="card border shadow-sm"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
    ...
    <tbody>@forelse($tasks as $task)<tr><td>{{ $task->patient?->patient_number }}</td><td><div class="fw-semibold">{{ $task->title }}</div><small class="text-muted">{{ $task->task_type }}</small></td><td><span class="badge bg-light text-dark">{{ $task->computed_status }}</span></td><td>{{ $task->due_at?->format('d M Y H:i') ?? '—' }}</td><td>{{ $task->assignedUser?->name ?? __('nursing.common.not_assigned') }}</td><td class="text-end"><a class="btn btn-sm btn-primary" href="{{ route('nursing.tasks.show', $task) }}">{{ __('nursing.common.view') }}</a></td></tr>@empty<tr><td colspan="6" class="text-center text-muted py-5">{{ __('nursing.tasks.empty') }}</td></tr>@endforelse</tbody>
