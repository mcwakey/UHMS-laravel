...
@section('content')
@php($treatmentShowRoute = ($workspaceContext['workspaceKey'] ?? null) === 'inpatient' ? 'inpatient.treatments.show' : 'nursing.treatments.show')
...
<div class="card border shadow-sm"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
    ...
    <tbody>@forelse($treatments as $treatment)<tr><td>{{ $treatment->patient?->patient_number }}</td><td>{{ $treatment->visit?->visit_number }}</td><td>{{ $treatment->type }}</td><td>{{ \Illuminate\Support\Str::limit($treatment->description, 80) }}</td><td class="text-end"><a class="btn btn-sm btn-primary" href="{{ route($treatmentShowRoute, $treatment->visit) }}">{{ __('nursing.common.view') }}</a></td></tr>@empty<tr><td colspan="5" class="text-center text-muted py-5">{{ __('nursing.treatments.empty') }}</td></tr>@endforelse</tbody>
