@extends('layouts.app')
@section('title', __('dashboards.admin_dashboard'))

@section('content')
<!-- Page Header -->
<div class="d-flex align-items-sm-center justify-content-between flex-wrap gap-2 mb-4">
    <div>
        <h4 class="fw-bold mb-0">{{ __('dashboards.admin_dashboard') }}</h4>
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('admin.patients.create') }}" class="btn btn-primary btn-sm">
            <i class="ti ti-user-plus me-1"></i>{{ __('dashboards.new_patient') }}
        </a>
        @endcan
        @can('visits.create')
        <a href="{{ route('admin.visits.create') }}" class="btn btn-success btn-sm">
            <i class="ti ti-calendar-plus me-1"></i>{{ __('dashboards.new_visit') }}
        </a>
        @endcan
        @can('appointments.create')
        <a href="{{ route('admin.appointments.create') }}" class="btn btn-info btn-sm text-white">
            <i class="ti ti-clock-plus me-1"></i>{{ __('dashboards.new_appointment') }}
        </a>
        @endcan
    </div>
</div>
        ...
        <div class="position-relative border card rounded-2 shadow-sm h-100">
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-2 justify-content-between">
                    ...
                    <a href="{{ route('admin.patients.index') }}" class="text-muted small">{{ __('common.view_all') }} →</a>
                </div>
                <p class="mb-1 text-muted small">{{ __('dashboards.total_patients') }}</p>
        ...
        <div class="position-relative border card rounded-2 shadow-sm h-100">
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-2 justify-content-between">
                    ...
                    <span class="text-muted small">{{ __('dashboards.this_month') }}</span>
                </div>
                <p class="mb-1 text-muted small">{{ __('dashboards.month_revenue') }}</p>
                <h3 class="fw-bold mb-0">&#8373;{{ number_format($dashboardStats['month_revenue'] ?? 0, 2) }}</h3>
                <small class="text-muted">{{ __('dashboards.today') }}: &#8373;{{ number_format($dashboardStats['today_revenue'] ?? 0, 2) }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-3 col-md-6">
        <div class="position-relative border card rounded-2 shadow-sm h-100">
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-2 justify-content-between">
                    ...
                    <a href="{{ route('admin.visits.index', ['today' => 1]) }}" class="text-muted small">{{ __('common.view_all') }} →</a>
                </div>
                <p class="mb-1 text-muted small">{{ __('dashboards.todays_visits') }}</p>
                <h3 class="fw-bold mb-0">{{ $visitStats['total'] ?? 0 }}</h3>
                <small class="text-muted">{{ __('dashboards.waiting_consulting', ['waiting' => $visitStats['waiting'] ?? 0, 'consulting' => $visitStats['consulting'] ?? 0]) }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-3 col-md-6">
        <div class="position-relative border card rounded-2 shadow-sm h-100">
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-2 justify-content-between">
                    ...
                    <a href="{{ route('admin.billing.invoices.index') }}" class="text-muted small">{{ __('common.view_all') }} →</a>
                </div>
                <p class="mb-1 text-muted small">{{ __('dashboards.outstanding_balance') }}</p>
    ...
    <div class="col-xl-2 col-md-4 col-6">
        <div class="card border-start border-info border-3 shadow-sm h-100">
            <div class="card-body py-3">
                <p class="text-muted mb-1 small">{{ __('dashboards.today_appointments') }}</p>
                <h4 class="fw-bold mb-0">{{ $dashboardStats['today_appointments'] ?? 0 }}</h4>
                <small class="text-info"><i class="ti ti-calendar-event me-1"></i>{{ __('dashboards.scheduled') }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-2 col-md-4 col-6">
        <div class="card border-start border-primary border-3 shadow-sm h-100">
            <div class="card-body py-3">
                <p class="text-muted mb-1 small">{{ __('dashboards.active_admissions') }}</p>
                <h4 class="fw-bold mb-0">{{ $dashboardStats['active_admissions'] ?? 0 }}</h4>
                <small class="text-primary"><i class="ti ti-bed me-1"></i>{{ __('dashboards.beds_used', ['count' => $dashboardStats['occupied_beds'] ?? 0]) }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-2 col-md-4 col-6">
        <div class="card border-start border-warning border-3 shadow-sm h-100">
            <div class="card-body py-3">
                <p class="text-muted mb-1 small">{{ __('dashboards.pending_prescriptions') }}</p>
                <h4 class="fw-bold mb-0">{{ $dashboardStats['pending_prescriptions'] ?? 0 }}</h4>
                <small class="text-warning"><i class="ti ti-prescription me-1"></i>{{ __('dashboards.awaiting_dispensing') }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-2 col-md-4 col-6">
        <div class="card border-start border-secondary border-3 shadow-sm h-100">
            <div class="card-body py-3">
                ...
                <small class="text-secondary"><i class="ti ti-test-pipe me-1"></i>{{ __('dashboards.requests') }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-2 col-md-4 col-6">
        <div class="card border-start border-danger border-3 shadow-sm h-100">
            <div class="card-body py-3">
                <p class="text-muted mb-1 small">{{ __('dashboards.pending_claims') }}</p>
                <h4 class="fw-bold mb-0">{{ $dashboardStats['pending_claims'] ?? 0 }}</h4>
                <small class="text-danger"><i class="ti ti-file-check me-1"></i>{{ __('dashboards.under_review') }}</small>
            </div>
        </div>
    </div>
    <div class="col-xl-2 col-md-4 col-6">
        <div class="card border-start border-success border-3 shadow-sm h-100">
            <div class="card-body py-3">
                <p class="text-muted mb-1 small">{{ __('dashboards.active_doctors') }}</p>
                <h4 class="fw-bold mb-0">{{ $dashboardStats['active_doctors'] ?? 0 }}</h4>
                <small class="text-success"><i class="ti ti-stethoscope me-1"></i>{{ __('dashboards.on_staff') }}</small>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-lg-8">
        <div class="card shadow-sm h-100">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0">{{ __('dashboards.revenue_visit_trends') }} <span class="text-muted fw-normal small">({{ __('dashboards.last_7_days') }})</span></h5>
            </div>
            <div class="card-body">
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm h-100">
            <div class="card-header">
                <h5 class="card-title mb-0">{{ __('dashboards.department_load') }} <span class="text-muted fw-normal small">({{ __('dashboards.today') }})</span></h5>
            </div>
            <div class="card-body">
                ...
                @else
                    <div class="text-center text-muted py-5">
                        <i class="ti ti-chart-donut fs-48 d-block mb-2"></i>{{ __('dashboards.no_department_activity') }}
                    </div>
                @endif
            </div>
        </div>
...
<div class="row g-3 mb-3">
    <div class="col-lg-4">
        <div class="card shadow-sm h-100">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0"><i class="ti ti-alert-triangle text-warning me-1"></i>{{ __('dashboards.low_stock_alerts') }}</h5>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-sm mb-0">
                        <thead class="table-light">
                            <tr><th>{{ __('common.drug') }}</th><th class="text-center">{{ __('common.qty') }}</th><th class="text-center">{{ __('dashboards.reorder') }}</th></tr>
                        </thead>
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="3"><x-empty-state :message="__('dashboards.no_low_stock')" /></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm h-100">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0"><i class="ti ti-skull text-danger me-1"></i>{{ __('dashboards.expired_stock') }}</h5>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover table-sm mb-0">
                        <thead class="table-light">
                            <tr><th>{{ __('common.drug') }}</th><th class="text-center">{{ __('common.qty') }}</th><th>{{ __('dashboards.expired') }}</th></tr>
                        </thead>
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="3"><x-empty-state :message="__('dashboards.no_expired_stock')" /></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm h-100">
            <div class="card-header">
                <h5 class="card-title mb-0">{{ __('dashboards.quick_stats') }}</h5>
            </div>
            <div class="card-body">
                <ul class="list-group list-group-flush">
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                        <span class="small"><i class="ti ti-flask text-info me-2"></i>{{ __('dashboards.today_admissions') }}</span>
                        <span class="badge bg-info rounded-pill">{{ $dashboardStats['today_admissions'] ?? 0 }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                        <span class="small"><i class="ti ti-users-group text-primary me-2"></i>{{ __('dashboards.total_users') }}</span>
                        <span class="badge bg-primary rounded-pill">{{ $totalUsers ?? 0 }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                        <span class="small"><i class="ti ti-user-check text-success me-2"></i>{{ __('dashboards.active_users') }}</span>
                        <span class="badge bg-success rounded-pill">{{ $activeUsers ?? 0 }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                        <span class="small"><i class="ti ti-building-bank text-warning me-2"></i>{{ __('dashboards.departments') }}</span>
                        <span class="badge bg-warning rounded-pill">{{ $totalDepartments ?? 0 }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                        <span class="small"><i class="ti ti-calendar-check text-success me-2"></i>{{ __('dashboards.today_completed') }}</span>
                        <span class="badge bg-success rounded-pill">{{ $visitStats['completed'] ?? 0 }}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2">
                        <span class="small"><i class="ti ti-ambulance text-danger me-2"></i>{{ __('dashboards.emergency_visits') }}</span>
...
<div class="row g-3 mb-3">
    <div class="col-lg-7">
        <div class="card shadow-sm">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0">{{ __('dashboards.todays_appointments') }}</h5>
                @can('appointments.view')
                <a href="{{ route('admin.appointments.index') }}" class="btn btn-sm btn-outline-primary">{{ __('common.view_all') }}</a>
                @endcan
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
    ...
    <div class="col-lg-5">
        <div class="card shadow-sm">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0">{{ __('dashboards.recent_patients') }}</h5>
                @can('patients.view')
                <a href="{{ route('admin.patients.index') }}" class="btn btn-sm btn-outline-primary">{{ __('common.view_all') }}</a>
                @endcan
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover table-sm mb-0">
                        <thead class="table-light">
                            <tr><th>{{ __('common.id') }}</th><th>{{ __('common.name') }}</th><th>{{ __('dashboards.registered') }}</th></tr>
                        </thead>
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="3"><x-empty-state :message="__('dashboards.no_patients_yet')" /></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
...
<div class="row g-3">
    <div class="col-12">
        <div class="card shadow-sm">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="card-title mb-0">{{ __('dashboards.todays_visits') }}</h5>
                @can('visits.view')
                <a href="{{ route('admin.visits.index', ['today' => 1]) }}" class="btn btn-sm btn-outline-primary">{{ __('common.view_all') }}</a>
                @endcan
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="7"><x-empty-state :message="__('dashboards.no_visits_today')" /></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const labels  = allDays.map(d => {
        ...
        return date.toLocaleDateString(@json(app()->getLocale() === 'fr' ? 'fr-FR' : 'en-GB'), { day: '2-digit', month: 'short' });
    });
    ...
    if (trendCtx) {
        new Chart(trendCtx, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: @json(__('dashboards.revenue')) + ' (\u20b5)',
                    ...
                    {
                        label: @json(__('dashboards.visits')),
            ...
            options: {
                ...
                scales: {
                    y:  { type: 'linear', display: true, position: 'left',  title: { display: true, text: @json(__('dashboards.revenue')) + ' (\u20b5)' } },
                    y1: { type: 'linear', display: true, position: 'right', title: { display: true, text: @json(__('dashboards.visits')) }, grid: { drawOnChartArea: false } }
                }
            }
        });
    }
